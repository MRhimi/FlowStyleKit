...
public final class TextFlowLabel: UILabel {
    ...
    private func updateText() {
        ...
        // MARK: - Apply highlight
        if !searchTerm.isEmpty, let fullText = result.string as NSString? {
            let regex = try? NSRegularExpression(
                pattern: NSRegularExpression.escapedPattern(for: searchTerm),
                options: .caseInsensitive
            )
            let range = NSRange(location: 0, length: fullText.length)
            regex?.enumerateMatches(in: fullText as String, options: [], range: range) { match, _, _ in
                if let matchRange = match?.range {
                    result.addAttribute(.foregroundColor, value: highlightColor, range: matchRange)
                }
            }
        }
